...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("select p from Payment p where p.reservationNumber = ?1 AND p.paidFor = 'RESERVATION' AND p.paid = false")
    Payment findNotPaidByReservationNumberAndReservationType(Long reservationNumber);

    @Query("select p from Payment p where p.reservationNumber = ?1 AND p.paidFor = 'RESERVATION' ORDER BY p.id DESC")
    List<Payment> findByReservationNumberAndReservationType(Long reservationNumber);
