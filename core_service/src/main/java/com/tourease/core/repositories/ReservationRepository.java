...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("select r from Reservation r where r.userId = :userId " +
            ...
            "and (cast(:checkIn as date) is null or r.checkIn BETWEEN :checkIn AND :checkInEnd) " +
            "ORDER BY r.creationDate DESC")
    Page<Reservation> findByUserId(Long userId, Long reservationNumber, OffsetDateTime creationDate, OffsetDateTime creationDateEnd, OffsetDateTime checkIn, OffsetDateTime checkInEnd, ReservationStatus status, Pageable pageable);
}
